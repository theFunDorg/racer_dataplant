...
while [[ $finish != "1" ]];
  do 
    ...
    echo "";
    echo "";
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep tickerplant |wc -l` == "1" ]]; then
    ...
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep rdb |wc -l` == "1" ]]; then
      echo "rdb not running, starting now";
      nohup /home/sean/q/l64/q skeleton.q -procName rdb >> /logOuts/rdb_`date +%Y.%m.%d_%H:%M:%S`.log 2>&1 &;
      sleep 3;
    fi;
  
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep engine |wc -l` == "1" ]]; then
    ...
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep subROS |wc -l` == "1" ]]; then
    ...
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep pubROS |wc -l` == "1" ]]; then
    ...
    if[[ `ps  -ef | grep "/home/sean/q/l64/q skeleton.q" | grep-v "grep" | grep logger |wc -l` == "1" ]]; then
